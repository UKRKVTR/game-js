...
function createClickCounter (button, maxClicks) {
  ...
  function handleClick () {
    if (clickCount < maxClicks) {
      clickCount++
      console.log(
        `Button "${button.id}" clicked (${clickCount}/${maxClicks} clicks)`
      )
    } else {
      console.log(
        `Button "${button.id}" reached maximum clicks (${maxClicks} clicks)`
      )
...
const fighter = {
  character: {
    ...
    changeHP (count) {
      const log =
        this === fighter.enemy
          ? generateLog(this, fighter.character, count)
          : generateLog(this, fighter.enemy, count)
